...
def test_to_dict():
    ...
    assert isinstance(Loan().to_dict(ordered=True), OrderedDict)
...
def test_summarize():
    for kind in ['amortized', 'interest_only']:
        ...
        if kind == 'amortized':
            expect = summarize_amortized_loan(loan.principal,
              loan.interest_rate, loan.compounding_freq,
              loan.payment_freq, loan.num_payments)
        elif kind == 'interest_only':
            expect = summarize_interest_only_loan(loan.principal,
              loan.interest_rate, loan.payment_freq, loan.num_payments)
